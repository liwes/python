...
def mkdir(path):
    ...
    isExists = os.path.exists(os.path.join("D:\python\download", path))
    # 如果不存在
    if not isExists:
        print('makedir', path)
        # 创建文件夹
        os.makedirs(os.path.join("D:\python\download", path))
        # 切换到创建的文件夹
        os.chdir(os.path.join("D:\python\download", path))
        return True
    # 如果存在了就返回False
    else:
...
def get_imgs():
    ...
    for url in all_page():
        ...
        imgConvent(allimgs)
        # print('allimgs = ',allimgs)
        #comment-4086566 > div > div > div.text
        #comment-4086566 > div > div > div.text > p > img
        #comment-4086509 > div > div > div.text
        #comment-4086566 > div > div > div.text > p > img
        #comment-4086509 > div > div > div.text > p > img
        # 调用download函数下载保存
        # download(allimgs)
    # 执行完毕打出ok
    print('ok')
